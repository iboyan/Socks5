...
resName="iboyan/Socks5/main"
serviceFile=/etc/opt/ss5/service.sh
newVersionMsg=`curl -s -L  https://raw.githubusercontent.com/${resName}/update.txt`
...
if [[ $choice == 5 ]];then
	...
	if [[ "$c" == "123" ]];then
	wget -q -N --no-check-certificate https://raw.githubusercontent.com/${resName}/install.sh && bash install.sh
	exit 0
	else
		clear
		bash $serviceFile
